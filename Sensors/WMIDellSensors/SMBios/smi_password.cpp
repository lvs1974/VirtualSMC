...
int verify_password(int which, const char *pass_scancode, u16 *security_key);
...
int dell_smi_get_security_key(const char *password, u16 *key)
{
    ...
    for (int i=0; i<numpass; i++)
    {
		...
		SYSLOG("sdell", "SMBIOS dell_smi_get_security_key: check %d", which);
        ...
        // if function succeeded and password *not* installed, skip
 		SYSLOG("sdell", "SMBIOS dell_smi_get_security_key: after get_password_properties_2: tmpret(%d)  p.installed(%d)", tmpret, p.installed);
		...
		SYSLOG("sdell", "SMBIOS dell_smi_get_security_key: after password_installed: tmpret(%d)", tmpret);
		...
		SYSLOG("sdell", "SMBIOS dell_smi_get_security_key: after password_installed: after verify_password_2: tmpret(%d)  security_key(%d)", tmpret, security_key);
		...
		SYSLOG("sdell", "SMBIOS dell_smi_get_security_key: after verify_password: tmpret(%d)  security_key(%d)\n", tmpret, security_key);
        if (tmpret==0) // correct, security key set
        {
            return_code = 0;
            goto out;
		...
		SYSLOG("sdell", "SMBIOS dell_smi_get_security_key: end of loop");
    }
...
out_free:
	SYSLOG("sdell", "SMBIOS get_password_properties_2: out_free");
    dell_smi_obj_free(smi);
out:
	SYSLOG("sdell", "SMBIOS get_password_properties_2: out");
    return retval;
}
